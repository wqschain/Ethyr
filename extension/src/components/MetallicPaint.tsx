...
export function parseLogoImage(
  ...
  return new Promise((resolve, reject) => {
    ...
    img.onload = function () {
      // For SVGs, we need to explicitly set dimensions
...
void main() {
    ...
    vec3 color = vec3(0.);
    float opacity = img.a;  // Use image alpha for opacity
    vec3 color1 = vec3(.98, 0.98, 1.);
    vec3 color2 = vec3(.1, .1, .1 + .1 * smoothstep(.7, 1.3, uv.x + uv.y));
    
    // Use red channel for edge detection since we converted to grayscale
    float edge = img.r;
    
    // Only apply effect where the image is not transparent
    if (img.a < 0.01) {
        edge = 1.0;
    }
...
export default function MetallicPaint({
  ...
  useEffect(() => {
    ...
    try {
      ...
      // Handle texture upload errors silently to avoid console noise
      // This can happen during normal operation when switching images
      return;
    }
